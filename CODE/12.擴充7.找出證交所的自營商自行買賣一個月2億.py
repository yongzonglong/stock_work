...
c1 = conn1.cursor()
c2 = conn2.cursor()
cursor =  c1.execute("select stock_id from ThreeBigSUM group by stock_id order by stock_id DESC")

for row in cursor:
    # print "準備進入",row[0].replace(" ","");
    cursor2 = c2.execute('select *,close_price*selfEmployed_sum from ThreeBigSUM where stock_id = "' + row[0].replace(" ","") + '"')
    mylist = list()
    dateMyList = list()
    for row2 in cursor2:
        # print row2[0],row2[7];
